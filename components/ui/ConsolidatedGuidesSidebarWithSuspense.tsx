'use client'

import { Suspense } from 'react'
import { ConsolidatedGuidesSidebar } from '@/components/ui/ConsolidatedGuidesSidebar'

interface SimpleGuide {
  slug: string
  title: string
  category: string | null
}

interface Props {
  guides: SimpleGuide[]
}

export function ConsolidatedGuidesSidebarWithSuspense({ guides }: Props) {
  return (
    <Suspense fallback={
      <aside className="w-full lg:w-80 lg:shrink-0 lg:border-r lg:border-border/60 bg-surface/30 lg:pr-6 h-fit lg:h-[calc(100vh-5rem)] lg:sticky lg:top-14 overflow-y-auto pb-16 space-y-6">
        <div className="space-y-1.5 pb-4 border-b border-border/40">
          <div className="h-3 w-24 bg-surface-variant rounded-md animate-pulse" />
          <div className="h-4 w-36 bg-surface-variant rounded-md animate-pulse" />
        </div>
        <div className="space-y-2">
          {Array.from({ length: 6 }).map((_, i) => (
            <div key={i} className="h-9 w-full bg-surface-variant rounded-xl animate-pulse" />
          ))}
        </div>
      </aside>
    }>
      <ConsolidatedGuidesSidebar guides={guides} />
    </Suspense>
  )
}
